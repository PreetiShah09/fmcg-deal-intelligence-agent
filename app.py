...
with st.sidebar:
    ...
    st.caption(
        "Relevant articles are kept as evidence; only articles with "
        "clear transaction patterns become Deal Monitor records."
    )
...
# Render AFTER refresh so timestamp is current.
st.markdown(
    f"**Last refreshed:** "
...
with tab_trace:
    ...
    if trace:
        ...
        st.info(
            "Run Refresh Now to see the agent trace."
        )
